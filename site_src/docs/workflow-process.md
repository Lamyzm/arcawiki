# 작업 프로세스 — 내 장비에서 그림 뽑기

> 이 문서만 다른 문서들과 성격이 다르다. 나머지가 **채널 원문 정리본**이라면,
> 이건 **이 PC(RTX 5070 Ti 16GB)에서 실제로 돌려 본 실행서**다.
> 숫자는 전부 실측이거나 원문 인용이고, 추측은 넣지 않았다.
>
> 정리 2026-08-22.
...
정리 전에는 워크플로우가 175개였다. 실사해 보니 **실제로 쓰는 것은 3개**였고
나머지는 이틀치 실험 로그와 그 복사본이었다.

| 폴더 | 개수 | 무엇 |
|---|---|---|
| **`00_ACTIVE`** | **3** | **여기만 보면 된다** |
| `01_ORIGINAL` | 2 | 채널 배포 원본. 개조본이 이상해지면 여기로 돌아온다 |
| `03_BROKEN` | 12 | 모델 파일이 없어서 못 도는 것. 받으면 살아난다 |
| `Getting Started` | 23 | ComfyUI 기본 샘플 |
| `_archive_2026-08` | 130 | 실험 잔해 |

```
00_ACTIVE/
  AiO_v60_local.json        ANIMA 올인원 · 템플릿   ← 주력
  AiO_run.json              ANIMA 올인원 · 실행본
  WAI_ILXL_T2I_local.json   WAI(SDXL) 계열
```

!!! note "채널 국룰은 5개다"
    가장 많이 배포되는 포터블 팩은 **ANIMA 심플 t2i / ANIMA t2i / SDXL·IL 올인원 /
    ANIMA 인페인팅 / SDXL·IL 인페인팅** 다섯으로 나눈다.
    나누는 기준은 **모델 계열(ANIMA vs SDXL)** 과 **큰 용도(생성 vs 인페인팅)** 두 가지뿐이다.
    그 외의 이유로 파일을 늘리지 않는다.
...
| **4** | **ANIMA 시리즈** · **NAI 4.5/V5** | **애니메 무검열. 우리 영역** |
| 5 | NoobAI / ILXL 기반 SDXL | "가볍고 빠르지만 **구식**" |

**ANIMA 와 NAI 가 같은 티어다.** 로컬 애니메 최상위를 이미 쓰고 있다는 뜻이다.
해외 추천 리스트가 미는 WAI·NoobAI·Illustrious 는 채널 기준 한 단계 아래다.
다만 "노하우가 많아 가볍게 쓸 만하다"고도 하며, 2인 영역분리는 실제로 이쪽으로 한다.

### 이 PC 에 있는 것 — 81개 파일 120.1GB

| 계열 | 크기 | 쓰나 |
|---|---|---|
| MiniMax H3 (영상) | 약 40GB | ✗ 워크플로우가 모델 이름 불일치로 깨져 있음 |
| Z-Image | 17.2GB | ✗ 애니메 특화 아님 |
| WAI Illustrious | 26GB (4벌) | ⭕ **1벌만 쓰면 됨** |
| **ANIMA** | 11.7GB (3벌) | ⭕ **`anima-aesthetic-v1.1` 주력** |
| SAM3 (마스킹) | 4.8GB | ⭕ 디테일러가 쓴다 |
| 텍스트 인코더 | 28.4GB | 부분 |

정리하며 **같은 파일이 두 경로에 있던 체크포인트 6.5GB** 를 걷어냈다.
`extra_model_paths.yaml` 이 `F:/AI/models` 를 `is_default: true` 로 잡고 있어
ComfyUI 목록에는 원래 1개로만 보이고 있었다.
...
우리 DB 집계(작가 사용 기록 1,921행 / 고유 916명) 기준이다.
...
!!! tip "작가를 그림 보고 고르기"
    ...
    **고정 프롬프트로 ANIMA 가 직접 생성한 이미지**다 — 즉 우리 모델에서 실제로 어떻게 나오는지 보여준다.
...
`AiO_v60_local.json` 은 노드 103개, 그룹 12개다. 그룹이 좌→우로 **단계 순서대로** 놓여 있다.
...
- **175개를 다 살리지 않는다.** *"Short answer, we don't lol."*
