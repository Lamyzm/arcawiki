...
**`qwen_3_4b...` / `z_image_turbo...` 누락이 뜨는 경우는 별개다.**
이건 대개 **ANIMA 설치가 깨진 것이 아니라 `Getting Started` 의 Z-Image 예제 워크플로우를 연 것**이다.
ANIMA 3파일만 넣어 둔 상태에서 Z-Image 예제를 열면 **정상적으로** 저 누락 경고가 뜬다.
즉 이 경우는 파일을 여기저기 다시 복사할 것이 아니라 **ANIMA 템플릿으로 돌아가서 다시 시작**하는 것이 맞다.
