...
**실제로 많이 헷갈리는 함정 하나** — 포터블의 `Getting Started` 폴더에는 **ANIMA 예제만 있는 것이 아니라 Z-Image 예제도 함께** 들어 있는 경우가 있다.
ANIMA 3파일만 넣어 둔 상태에서 Z-Image 예제를 열면 `qwen_3_4b...`, `z_image_turbo...` 누락 경고가 뜬다.
이건 **설치 실패가 아니라 워크플로우-모델 계열 불일치**다. ANIMA 를 먼저 검증할 때는 **Z-Image 예제를 고치지 말고 ANIMA 템플릿으로 돌아간다.**
...
### 지금 바로 한 장 뽑는 로컬 검증 루트

문서만 읽고 끝내지 말고 **여기서 바로 한 장이 나와야 한다.**
현재 로컬 검증에서 가장 안정적으로 통과한 출발점은 아래다.
...
| 스타일 LoRA | `ANIMA_STYLE_LORA\13.safetensors` `0.7` |
| 샘플러 | `er_sde` + `simple` |
| 스텝 / CFG | `30 steps / CFG 4.0` |
| 해상도 | `1024x1536` |
| 보조 태그 | `anime screencap`, `tv anime still`, `cel shading`, `flat colors` |

이 조합이 좋은 이유는 단순하다.

- **ANIMA 기본기**는 유지하고
- `13` 번 스타일 LoRA 가 **광택을 과하게 올리지 않으면서**
- `42` 번처럼 포스터풍 AI 일러 쪽으로 밀리지 않고
- `anime screencap` 보조와도 잘 맞았다

반대로 **광택이 너무 강하고 "AI 일러" 티가 심하면** `42` 류 스타일 LoRA 부터 의심하고,
**너무 밋밋해지면** `13` 대신 `4` 또는 `166745889` 로 바꿔 본다.
