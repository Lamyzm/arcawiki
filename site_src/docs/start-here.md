...
| **로컬 오픈웨이트** | 자기 PC 에서 돈다. 무료·무검열 | **ANIMA** · Illustrious/NoobAI · Z-Image · Flux · Chroma |
...
| **4** | **ANIMA** · **NAI 4.5/V5** | **애니메 무검열. 로컬로 갈 곳** |
...
채널이 배포하는 구성을 그대로 따른다. **무엇이 있어야 하는지**가 기준이다.
...
브라우저에서 `http://127.0.0.1:8188` 을 연다.
...
| 방법 | 어디 |
|---|---|
| **한국어로 검색** | `ComfyUI-Autocomplete-Plus` — 한국어 별칭 검색 |
| 사이트에서 찾기 | `taghelper.baepoyong.com` — 단부루 프롬프트 도우미 |
| 남의 그림에서 뽑기 | `aitag.win` — 픽시브 그림의 프롬프트 수집 |
| 아카 링크에서 뽑기 | 원본 PNG 의 `data-originalurl` 에서 메타데이터를 읽는다 (아래 9절) |
...
| 곳 | 규모 |
|---|---|
| **`animastyles.thetacursed.com`** | **작가 42,509명.** 프리뷰를 ANIMA 로 직접 생성한 것이라 로컬 결과와 같은 기준이다 |
...
`extra_model_paths.yaml` 로 공유 폴더를 추가로 잡아 뒀다면 **양쪽 다 읽는다.**
한쪽만 보고 "로라가 이것뿐"이라고 판단하면 안 된다.

계열별로 폴더를 나눠 두는 것이 채널 관행이다 — 그래야 목록에서 구분된다.

```
loras/ANIMA_STYLE/     화풍
loras/CHARACTER/       캐릭터
loras/WEBTOON/         웹툰체
```
...
원본 PNG 에 박힌 프롬프트·시드·크기를 뽑아 로컬 모델 문법으로 변환해 바로 생성한다.
